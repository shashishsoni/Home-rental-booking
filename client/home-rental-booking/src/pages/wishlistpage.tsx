...
const WishlistPage = () => {
  ...
  const allListings = useSelector((state: any) => state.user?.listings || []);
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      try {
        ...
        const parsedSavedListings = savedListings ? JSON.parse(savedListings) : [];
        ...
        const missingListings = wishlist.filter((id: string) => 
        ...
        if (missingListings.length > 0) {
          const newListings = await Promise.all(
            missingListings.map(async (listingId: string) => {
              try {
                ...
                return {
                  ...listing,
                  ListingPhotoPaths: listing.listingImages?.map((path: string) => {
                    const cleanPath = path.replace(/^\/?(public\/)?/, '');
                    return `http://localhost:3001/${cleanPath}`;
                  }) || [],
                  creator: {
                    _id: listing.Creator?._id || listing.creator?._id,
                    firstname: listing.Creator?.firstname || listing.creator?.firstname || 'Unknown',
                    lastname: listing.Creator?.lastname || listing.creator?.lastname || 'Host',
                    Email: listing.Creator?.Email || listing.creator?.Email || '',
          ...
          validNewListings.forEach(newListing => {
            if (!allValidListings.some(existing => existing._id === newListing._id)) {
              allValidListings.push(newListing);
            }
          });

          // Update both localStorage and Redux
          localStorage.setItem('wishlistListings', JSON.stringify(allValidListings));
          ...
          {selectedListing && (
            ...
            >
              ...
              <div className="relative h-64 w-full">
                {selectedListing?.ListingPhotoPaths && selectedListing.ListingPhotoPaths.length > 0 ? (
                  <img
              ...
              <div className="p-6">
                <div className="flex items-center space-x-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-white">
                      {selectedListing.creator?.firstname} {selectedListing.creator?.lastname}
                    </h3>
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-white/5 rounded-lg">
                    <h4 className="text-lg font-semibold text-white mb-2">Property Details</h4>
                    <div className="space-y-2 text-gray-300">
                      ...
                      <p className="flex items-center">
                        ...
                        <span className="ml-2">{selectedListing.city}, {selectedListing.country}</span>
                      </p>
                      <p className="flex items-center">
